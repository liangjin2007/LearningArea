...
- 着色模型Shading Model
  - Diffuse BRDF 
    ...
    - Microfacet Specular BRDF $f(\vec{l}, \vec{v}) = \frac {D\left(\vec{h}\right) F\left(\vec{v}, \vec{h}\right) G\left(\vec{l}, \vec{v}, \vec{h}\right)}{4\left(\vec{n} \dot \vec{l}\right)\left(\vec{n} \dot \vec{v}\right)}$
